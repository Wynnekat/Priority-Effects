...
#Working directory that contains datasets
setwd("~/Desktop/PhD Research/Priority Effects - 2021/Datasets/Cover_Datasets")

#Import datasets

## inner (1 m^2) cover for summer 2022
#### comprehensive vegetative cover (%) for all species in the inner (1 m^2) permanent sampling area. 

inner_cover_summer_2022 <- read_excel("PE_Cover_Inner_Summer_2022_Cleaned.xlsx")

inner_cover_summer_2023 <- read_excel("PE_Cover_Inner_Summer_2023_Cleaned.xlsx")

## inner (1 m^2) cover for fall 2022
#### comprehensive vegetative cover (%) for all species in the inner (1 m^2) permanent sampling area. 

inner_cover_fall_2022 <- read_excel("PE_Cover_Inner_Fall_2022_Cleaned.xlsx")


## Bradford weather
weather <- read_excel("Bradford_10_Year_Weather.xlsx")
...
### Fully join both datasets together
Full_data <- full_join(inner_cover_summer_2022,inner_cover_fall_2022)


### Remove season and year
...
for(i in 1:nrow(Full_data)){
  ...
  if(Full_data[i,3] == "CARFRA"){Full_data [i,3] <- "CARSPP"}
  if(Full_data [i,3] == "CARBRE"){Full_data [i,3] <- "CARSPP"}
  if(Full_data [i,5] == "Carex brevior"){Full_data [i,5] <- "Carex spp."}
  if(Full_data [i,5] == "Carex frankii"){Full_data [i,5] <- "Carex spp."}
  if(Full_data [i,5] == "Heuchera richardsonii"){Full_data [i,5] <- "Geum canadense"}
   if(Full_data [i,3] == "HEURIC"){Full_data [i,3] <- "GEUCAN"}
}  
  
### Group by and then select the SPP6 with the max cover 
inner_cover_max_2022 <- Full_data  %>%
      group_by(Block, Treatment, SPP6, Scientific_Name) %>%
...
### Remove VERSPP since it was < than VERARV for all the same locations
inner_cover_max_2022 <- subset(inner_cover_max_2022, SPP6 != "VERSPP")
### Remove Bare
inner_cover_max_2022 <- subset(inner_cover_max_2022, SPP6 != "BARE")
### Remove Litter
inner_cover_max_2022 <- subset(inner_cover_max_2022, SPP6 != "LITTER")
```

```{r, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}

#Checking again to make sure all the SPP6 levels are correct (no misspellings or weirdness)
sort(unique(inner_cover_max_2022$SPP6))

#Checking again to make sure all the scientific names are correct (no misspellings or weirdness)
sort(unique(inner_cover_max_2022$Scientific_Name))


### Saw at least 73 different species in 2022
### Number of scientific names = number of SPP6 codes (yay :) )

#View(inner_cover_max_2022)
```
...
### c values and provenance list for Missouri species
#### N/A is N = native, A = non-native, G = Genus
setwd("~/Desktop/PhD Research/Priority Effects - 2021/Datasets/Cover_Datasets")

# Read in the dataset with of all the Missouri Flora and associated info
checklist_Missouri_flora <- read_excel("Checklist of Missouri Flora.xlsx")

# Make the C values a number
checklist_Missouri_flora$C <- as.numeric(checklist_Missouri_flora$C)

# Change the column names to match how I named my datasets
checklist_Missouri_flora <- checklist_Missouri_flora %>% 
  rename(SPP6 = ACRON) %>% 
  rename(Scientific_Name = `SCIENTIFIC NAME`)

# Change the Latin names to lower case so I can join the two datasets later on
checklist_Missouri_flora$Scientific_Name <- tolower(checklist_Missouri_flora$Scientific_Name)

#### ***Note**** made some decisions here
####   SPHOBO = SPHOBT and changed the name to match
####   Changed Solidago canadensis var. hargeri to Solidago canadensis 
####   Melilotus officinale is not an accepted name and I changed it to M. officinalis
####   Setaria glauca is not an accepted name and I changed it to Seteria pumila (along with the code)
####   CONCAC = CONCAN and changed the name to match


for(i in 1:nrow(checklist_Missouri_flora)){
  
  if(checklist_Missouri_flora[i,4] == "sphenopholis obtusata var. obtusata"){checklist_Missouri_flora[i,4] <- "sphenopholis obtusata"}
  if(checklist_Missouri_flora[i,1] == "SPHOBO"){checklist_Missouri_flora[i,1] <- "SPHOBT"}
  if(checklist_Missouri_flora[i,4] == "melilotus officinale"){checklist_Missouri_flora[i,4] <- "melilotus officinalis"}
  if(checklist_Missouri_flora [i,4] == "conyza canadensis var. canadensis"){checklist_Missouri_flora[i,4] <- "conyza canadensis"}
  if(checklist_Missouri_flora [i,1] == "CONCAC"){checklist_Missouri_flora[i,1] <- "CONCAC"}
  if(checklist_Missouri_flora [i,4] == "solidago canadensis var. hargeri"){checklist_Missouri_flora[i,4] <- "solidago canadensis"}
  if(checklist_Missouri_flora [i,4] == "setaria glauca"){checklist_Missouri_flora[i,4] <- "setaria pumila"}
  if(checklist_Missouri_flora [i,1] == "SETGLA"){checklist_Missouri_flora[i,1] <- "SETPUM"}
...
### Making a Species List - 2022


#### Take inner cover data then group by species (code and scientific name)
#### Sum by cover to obtain new dataset that contains a list of the species recorded in inner study plots

PE_2022_Species_List <- inner_cover_max_2022 %>% 
  group_by(SPP6, Scientific_Name) %>% 
  summarize(Percent_Cover = sum(max_cover))

### Remove the percent cover column to get just a list of species 
PE_2022_Species_List <- PE_2022_Species_List[, -3]

## Convert all names to lower case for the join
PE_2022_Species_List$Scientific_Name <- tolower(PE_2022_Species_List$Scientific_Name)
  
### Join the missouri flora checklist to my species list

PE_2022_Species_List_Detailed <- left_join(PE_2022_Species_List, checklist_Missouri_flora, by = "Scientific_Name")

### Export to edit in Excel
setwd("~/Desktop/PhD Research/Priority Effects - 2021/Datasets/Cover_Datasets")

#### Made the last species list dataset on December 06, 2022
#write_xlsx(PE_2022_Species_List_Detailed, "PE_Detailed_Species_List_2022.xlsx")
...
Species_Block_Plot_List <- inner_cover_max_2022 %>% 
  unite("UniqueBlockTreat", Block:Treatment, sep = "_", remove = FALSE) %>% 
  group_by(Block, Treatment, UniqueBlockTreat, SPP6, Scientific_Name)  %>% 
  summarize(totcover= sum(max_cover))


#write_xlsx(Species_Block_Plot_List, "Species_Block_Plot_List.xlsx")
```


```{r, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}

#Import cleaned species list to join with the original inner dataset
#### Last cleaned the dataset on December 06, 2022
setwd("~/Desktop/PhD Research/Priority Effects - 2021/Datasets/Cover_Datasets")


Species_List_PE_2022 <- read_excel("Cleaned_PE_Detailed_Species_List_2022.xlsx")

#Remove unnecessary columns
Species_List_Reduced_PE_2022 <- Species_List_PE_2022 [, 1:4]
Species_List_Reduced_PE_2022 <- Species_List_Reduced_PE_2022 %>% 
  rename(SPP6 = SPP6.x)

#Join to inner dataset
joined_inner_cover_2022 <- full_join(inner_cover_max_2022, Species_List_Reduced_PE_2022, by = "SPP6" )

#Remove extra name column
joined_inner_cover_2022 <- joined_inner_cover_2022[,-6]

#Rename the Scientific_Name column
joined_inner_cover_2022 <- joined_inner_cover_2022 %>% 
  rename(Scientific_Name = Scientific_Name.x)

#Check 
#view(joined_inner_cover_2022)
...
joined_inner_cover_2022$UniqueBlock <- paste(joined_inner_cover_2022$Treatment, joined_inner_cover_2022$Block, sep="_")
```

```{r, echo = FALSE, results = 'hide', message = FALSE, warning = FALSE}

#Export this cleaned 2022 data set as an excel file so I can use it for combined analyses with the 2021 data
## Last exported December 7th, 2022
setwd("~/Desktop/PhD Research/Priority Effects - 2021/Datasets/Cover_Datasets")
#write_xlsx(joined_inner_cover_2022, "joined_inner_cover_2022.xlsx")
...
joined_inner_cover_2022_reduced <- joined_inner_cover_2022[, c(3,5,8)]

joined_inner_cover_2022_reduced <- joined_inner_cover_2022_reduced %>% 
  group_by(UniqueBlock, SPP6) %>% 
  summarise(max_cover = sum(max_cover))


#Make a wide formatted dataset
inner_wide <- joined_inner_cover_2022_reduced  %>% 
...
inner_wide.labs <- inner_wide[, 1]

#Turn our dataset into a matrix 
inner_wide_mat <- inner_wide[,-1]

row.names(inner_wide_mat) <- inner_wide.labs$UniqueBlock
inner_wide_mat <- data.matrix(inner_wide_mat)
...
Species_richness <- specnumber(inner_wide_mat[,-1])
Species_richness
...
C_hist <- joined_inner_cover_2022  %>% 
                  filter( Provenance %in% "N") %>% 
                  group_by(Treatment, SPP6, C) %>% 
...
Mean_C_2022 <- joined_inner_cover_2022  %>% 
                  filter( Provenance %in% "N") %>% 
                  group_by(UniqueBlock, Treatment, Block) %>% 
...
Summary_table_2022 <- data.frame(inner_wide.labs$UniqueBlock, Mean_C_2022$Treatment, Mean_C_2022$Block, Species_richness, Shannon, Mean_C_2022$Mean_C)

# Summary table for the Mean C, Species Richness, and Shannon's diversity for each block and treatment combination

Summary_table_2022 <- Summary_table_2022 %>% 
  rename(UniqueBlock = inner_wide.labs.UniqueBlock, Treatment = Mean_C_2022.Treatment, Block = Mean_C_2022.Block,Mean_C = Mean_C_2022.Mean_C)

### Get rid of the row lables
rownames(Summary_table_2022) <- NULL
...
Summary_table_2022_mean <- Summary_table_2022 %>% 
  group_by(Treatment) %>%   
...
Summary_table_2022_mean <- Summary_table_2022_mean %>% 
